 ...
 */
export const bnameTofname = (bname: string, ext: string = "csv") =>
  `${bname}.${ext}`;
...
export const hashId = (id: string): string => {
  if (!hashIdCache[id]) hashIdCache[id] = Bun.hash(id).toString();
...
export const recordsCreatedHandler = (
  ...
  result.reduce<{ success: any[]; failure: string[] }>(
    (p, c) => {
      if (c.status === "rejected") p.failure.push(<string>c.reason);
      else p.success.push(c.value);
